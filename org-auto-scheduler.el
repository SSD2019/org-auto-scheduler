...
(defun org-auto-scheduler-save-review-decisions ()
  "Save review decisions to `org-auto-scheduler-review-state-file`."
  (when (memq org-auto-scheduler-review-persistence-backend '(state-file both))
...
(defun org-auto-scheduler-load-review-decisions (&optional force)
  "Load review decisions from `org-auto-scheduler-review-state-file`.
When FORCE is nil and `org-auto-scheduler--saved-review-decisions` is non-nil,
do nothing to preserve in-memory state."
  (when (and (or force (null org-auto-scheduler--saved-review-decisions))
             (memq org-auto-scheduler-review-persistence-backend '(state-file both))
...
SCOPE can be:
  `all`     - Clear all saved ordering and skipping decisions.
    ...
    ('all
     ...
     (when (memq org-auto-scheduler-review-persistence-backend '(org-properties both))
       (org-map-entries
        ...
        "+AUTOSCH" 'agenda))
     (org-auto-scheduler-save-review-decisions)
     (message "Cleared all saved review decisions."))
    ('skipped
               ...
               (when time-str
                 (let* ((time-val (org-time-string-to-time time-str))
                   ...
                   (let* ((date-string (format-time-string "%Y-%m-%d" time-val))
                          (item (list task-id time-val task-end-time tags t task-name has-time-flag (point-marker)))
              ...
              (when (and scheduled-time
                ...
                (let* ((scheduled-date (format-time-string "%Y-%m-%d" scheduled-time))
                  ...
                  (when (string= scheduled-date date-string)
                    (list task-id scheduled-time task-end-time tags t task-name has-time-flag (point-marker)))))))
          nil
          'agenda)
...
In calendar view, or on fixed agenda events, jumps to the entry instead."
  (interactive)
  ...
  (let* ((task-id (nth 0 event))
    ...
    (list row-id
          (vector ""
                  ...
                  (propertize dur-str 'face 'shadow)
                  (propertize tag-str 'face 'shadow)
                  (propertize "—" 'face 'shadow)
                  (propertize "🔒" 'face 'shadow)))))
...
and optionally existing fixed agenda events interleaved chronologically."
  ...
  (let* ((sorted-tasks (sort (copy-sequence tasks)
    ...
    (cons (list "__header_shortcuts"
                (vector "" "" (propertize "  [RET] toggle  [K/J] reorder  [>/<] day  [d] date  [r] recalc  [S] save  [M] merge  [C] clear  [x] apply  [?] help" 'face 'shadow)
                        "" "" "" "" ""))
...
(defun org-auto-scheduler-review-help ()
  ...
  (with-output-to-temp-buffer "*Org Auto Scheduler Review Help*"
    (with-current-buffer standard-output
      ...
      (insert "  C, C-c C-d   Clear saved decisions (all, skipped, order, or task at point)\n")
