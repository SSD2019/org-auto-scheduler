...
| =U= / =p=   | =review-move-up=                     | Move the current task up in the list (manually reorder).                       |
| =D= / =n=   | =review-move-down=                   | Move the current task down in the list (manually reorder).                     |
| =r= / =C-c C-r=| =review-recalculate=               | Recalculate proposed times from the *current visual order*, without resorting. |
