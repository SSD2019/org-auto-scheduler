...
- *Interactive Review Buffer:* Preview, reorder, toggle, and approve proposed schedules alongside existing fixed calendar events before applying.
...
| =org-auto-scheduler-server-start= ⭐                | Start local HTTP bridge server for mobile/Android app access.                               |
| =org-auto-scheduler-server-stop=                    | Stop the local HTTP bridge server.                                                          |
...
** 📱 Android Mobile App & Emacs Server Bridge
<<android-mobile-app>>

Org Auto Scheduler includes a feature-complete Android mobile application and local Emacs HTTP bridge server, allowing you to manage your schedule from your phone or tablet on your local Wi-Fi or Tailscale/Wireguard VPN.

*** Features at a Glance:
- *Agenda Timeline:* View scheduled tasks, effort estimates, and fixed calendar events.
- *Quick Agenda Bump:* Single-tap bump (+15m, +30m, +1h) to cascade-push today's remaining tasks when work runs overtime.
- *Interactive Review & Apply:* Full mobile recreation of the Emacs review buffer:
  - Toggle =[X]= / =[ ]= apply marks.
  - Touch reordering (Move Up, Move Down, Move Day forward/backward).
  - Inline / Modal What-If Effort editing and recalculation.
  - Bulk actions (mark all, unmark all, mark today, filter today).
  - Dependency blocker violation alerts and safe apply.
  - One-tap *Apply Schedule* to update your Org files directly.
- *Task Property Editor (Bottom Sheet):*
  - Modify effort estimates with quick presets (15m, 30m, 45m, 1h, 1.5h, 2h, custom).
  - Select TODO states (dynamically loaded from your Emacs configuration).
  - Set priorities ([#A], [#B], [#C]), scheduled dates, and deadlines.
  - Add/remove tags (=AUTOSCH=, =buffertime=, etc.).
  - Edit properties drawer (=NOT_BEFORE=, =BLOCKER=, =DEPENDS_ON=, =RECURRING=, custom).
  - Live task clocking (Clock In / Clock Out) and "Open Task in Emacs" jump.
- *Schedule Adherence:* Capture morning snapshots and view adherence streaks directly on your phone.

*** Starting the Server:

From Emacs:
#+begin_src emacs-lisp
M-x org-auto-scheduler-server-start
#+end_src

Or from the command line:
#+begin_src shell
python3 server/org_auto_scheduler_server.py --port 8989 --host 0.0.0.0
#+end_src

*** Connecting from Android:

1. *Instant PWA Install (Recommended):*
   - Connect your phone to the same Wi-Fi or Tailscale network.
   - Open =http://<your-computer-ip>:8989= in Chrome or Brave on Android.
   - Tap the browser menu and select *Install App* or *Add to Home Screen*.
   - The app runs in full-screen standalone mode with native haptics, dark theme, and offline shell caching.

2. *Native Android Studio Project:*
   - The =android/= directory contains a complete Gradle wrapper project ready to build an APK via Android Studio or =./gradlew assembleDebug=.
...
| =●=         | Color-coded dot representing the project.                      |
...
| =Status=    | Status indicator: =✓= (success), =⚠= (warning), =✗= (failed).  |

*** Keybindings

| Key         | Command                              | Action                                                                         |
|-------------+--------------------------------------+--------------------------------------------------------------------------------|
| =SPC= / =m= | =review-toggle=                      | Toggle the =[X]= / =[ ]= apply mark and advance one line.                      |
| =TAB= / =RET=| =review-jump=                       | Open the original Org task in another window.                                  |
| =U= / =p= / =K= | =review-move-up=                 | Move current task up in the list (crosses day separators into previous day).  |
| =D= / =n= / =J= | =review-move-down=               | Move current task down in the list (crosses day separators into next day).    |
...
| =r= / =C-c C-r=| =review-recalculate=               | Recalculate proposed times from visual order (auto-compacts into earlier slots; =C-u r= keeps day sections). |
| =R=         | =review-refresh=                     | Discard and re-run the full scheduler from scratch (picks up Org file changes).|
| =u=         | =review-undo=                        | Undo the last modification (toggle, reorder, effort edit, mark, or filter).   |
| =e=         | =review-edit-effort=                 | Edit estimated effort of task at point in-memory (What-If planning).           |
| =E=         | =review-toggle-agenda-events=        | Toggle showing existing fixed agenda/calendar events in review list.           |
| =v=         | =review-toggle-calendar=             | Toggle between the Column Table and the multi-day Chronological Calendar View. |
| =x= / =C-c C-c=| =review-execute=                   | Recalculate, then apply all =[X]= tasks and close the buffer.                  |
| =?=         | =review-help=                        | Show help buffer detailing all review commands.                                |
