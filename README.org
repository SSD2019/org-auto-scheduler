...
| =M-x org-auto-scheduler-snapshot-schedule=      | Record a snapshot of today's scheduled tasks for adherence tracking.                               |
| =M-x org-auto-scheduler-score-schedule=         | Score today's schedule adherence against the morning's snapshot.                                   |
| =M-x org-auto-scheduler-adherence-report=       | Display the adherence report buffer for today (or a specific date).                                |
...
1. *The Morning Snapshot*: Run =M-x org-auto-scheduler-snapshot-schedule= when you have your daily schedule ready. This takes a snapshot of all scheduled tasks planned for the day (loads automatically on startup).
2. *The Evening Score*: At the end of the day, run =M-x org-auto-scheduler-score-schedule=. This compares the morning snapshot to your actual completed work.

The scoring algorithm features:
- *Time-Gated Midday Scoring:* You can safely score yourself in the middle of the day. The algorithm will only consider tasks whose =SCHEDULED= start-time has already passed, allowing you to see your live pace without being penalized for afternoon tasks.
- *Retroactive Scoring:* Forgot to score yesterday? No problem! Call =M-x org-auto-scheduler-score-schedule= with a prefix argument (e.g. =C-u M-x...=) or interactively answer the prompt to score and record any day from the past 30 days.
- *Category Breakdown:* See adherence percentages split by category.
- *Partial Credit:* Giving you credit for effort spent even if the task isn't marked =DONE= (requires =org-clock=).
...
You can display a live, running adherence score directly in your Emacs mode line. It updates automatically every 5 minutes.

#+begin_src emacs-lisp
(org-auto-scheduler-adherence-mode-line-enable)
#+end_src

To disable it, run =M-x org-auto-scheduler-adherence-mode-line-disable=.
