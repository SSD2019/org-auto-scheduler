...
- [[#review-and-apply-buffer][Review & Apply Buffer]]
  ...
  - [[#visual-daily-capacity-gauge][Visual Daily Capacity Gauge]]
  ...
  - [[#in-buffer-task-slicer-chop][In-Buffer Task Slicer ("Chop")]]
  - [[#visual-what-if-diff-buffer][Visual What-If Diff Buffer]]
...
- [[#schedule-adherence-tracking][Schedule Adherence Tracking]]
  - [[#weekly-retrospective-summary][Weekly Retrospective Summary]]
...
*** 🎯 =M-x org-auto-scheduler-focus=

Launch the *Focus HUD* monotasking cockpit. Locks your attention on the currently active or scheduled task with real-time pacing, Pomodoro tracking, and in-cockpit checklist, note, subtask, and sibling capture.
...
** 🎯 Focus HUD Mode (Monotasking Cockpit)

The *Focus HUD* (=*Org Focus HUD*=) is designed specifically to prevent context switching, decision fatigue, and schedule drift. It displays *only* the single current task (with zero future task distractions) and turns your editor into a high-flow productivity cockpit.
...
,*Keybindings:*
- *Global Leader (Spacemacs):* ~SPC o S f~ or ~SPC o S F~
- *Org Buffer Major Mode:* ~SPC m d f~ or ~, d f~
- *Org Agenda:* ~C-c a F~ (leaves built-in follow mode on ~F~ intact)
- *Dispatch Menu:* ~SPC o S d~ → ~f~ (or ~S~ in Agenda → ~f~)

*** Visual Cockpit Layout

#+begin_example
╭─────────────────────────────────────────────────────────────────────────────╮
│ 🎯 FOCUS: Implement OAuth 2.0 PKCE Callback Handler     [14:25 · Tue Sep 25] │
╰─────────────────────────────────────────────────────────────────────────────╯

  PROJECT:  Backend / Auth Migration
  SLOT:     14:00 – 15:00 (Today) · Effort: 60m · Pomodoro: 🍅 [25m/5m]

  TIME REMAINING: 35m left
  [████████████████████░░░░░░░░░░░░░░░░░░░░░░░░] 25m clocked (41%)

  ┌─ CHECKLIST [2/4] ─────────────────────────────────────────────────────────┐
  │ [X] Generate PKCE code verifier and challenge (base64url)                 │
  │ [X] Add redirect URL validation with state token                          │
  │ [ ] Handle error callback when state parameter is invalid                 │
  │ [ ] Write unit tests for expired state token and replay attacks           │
  └───────────────────────────────────────────────────────────────────────────┘

  ┌─ SUBTASKS (CHILD TODOS) ──────────────────────────────────────────────────┐
  │ • [TODO] Implement token exchange endpoint POST /oauth/token              │
  │ • [TODO] Store refresh tokens securely in redis cache                     │
  └───────────────────────────────────────────────────────────────────────────┘

  ┌─ RECENT NOTES ────────────────────────────────────────────────────────────┐
  │ [14:18] Remember to use constant-time comparison for token hashes         │
  │ [14:05] RFC 7636 Section 4.3 requires SHA-256 for code_challenge_method   │
  └───────────────────────────────────────────────────────────────────────────┘

  CAPTURE (Zero context switching)   ACTIONS & PACING
  ─────────────────────────────────  ──────────────────────────────────────────
  [k] + Checklist item               [RET] Toggle checklist item [ ] ↔ [X]
  [n] + Quick Note                   [d]   Mark DONE & Advance
  [s] + Child Subtask                [+]   Extend +15m
  [a] + Sibling Task (after this)    [p]   Pause / Resume Clock
  [o] Jump to Org File               [q]   Minimize HUD
#+end_example

*** Keybindings & In-Cockpit Actions

| Key | Action | Description |
|-----+--------+-------------|
| =TAB= / =S-TAB= | Next/Prev Checklist | Navigate directly between checklist items in the HUD |
| =k= | Add Checklist | Prompt for text and insert =- [ ] <text>= under current task in Org |
| =RET= / =SPC= | Toggle Checklist | Toggle checkbox =[ ]= ↔ =[X]= in the underlying Org buffer |
| =n= | Add Note | Prompt for note and append timestamped entry =-[HH:MM] <note>= in drawer |
| =s= | Add Subtask | Create child headline =** TODO <title>= with optional effort |
| =a= | Add Sibling | Create sibling task =* TODO <title> :AUTOSCH:= directly after current task |
| =d= | Mark DONE & Advance | Mark task =DONE=, clock out, and auto-clock into the next scheduled task |
| =+= / === | Extend +15m | Extend current task effort and slot by 15m; ripples downstream tasks |
| =p= | Pause / Resume | Toggle =org-clock-out= / =org-clock-in= |
| =o= | Jump to Org | Open original Org file and reveal headline |
| =c= | Clock in Task | Select and clock into any scheduled task for today |
| =g= | Refresh | Force re-render of the HUD |
| =q= | Minimize | Dismiss Focus HUD window (clock stays active) |
...
*** Visual Daily Capacity Gauge
<<visual-daily-capacity-gauge>>

The review buffer header line renders a real-time Unicode capacity bar dynamically calculating your total scheduled workload against your configured workday duration:
#+begin_example
Load: [██████░░░░] 4.8h/8.0h (60%) │ Slack: 3.2h │ 6 tasks (4.8h)
#+end_example
- *Dynamic Visual Feedback:* Re-computed on the fly whenever tasks are reordered, chopped, deferred, or skipped.
- *Color-Coded Thresholds:*
  - =success= (Green): Normal capacity (<= 85%).
  - =warning= (Yellow/Orange): Near capacity (> 85% and <= 100%).
  - =error= (Red bold): Overload (> 100%), rendering =Load: [██████████] 9.5h/8.0h (119% OVERLOAD +1.5h)=.
...
*** In-Buffer Task Slicer ("Chop")
<<in-buffer-task-slicer-chop>>

When a large task cannot fit into today's remaining hours, you can slice it directly in the review buffer:
- *Keybinding:* Press =c= (or =C-c c=) on any task in the review table.
- *Interactive Prompt:* Prompts for minutes to keep today (e.g. =Keep today for 'Big Task' (current 120m, default 60m): =).
- *Automatic Deferral:* Automatically keeps your chosen duration for today, marks the task splittable, and defers remaining effort to subsequent days via clean placeholder subtasks.
- *Full Undo Support:* Press =u= or =U= anytime to revert the chop and restore the task's original full duration.

*** Visual What-If Diff Buffer
<<visual-what-if-diff-buffer>>

Before committing changes to your Org files with =x=, inspect a complete What-If diff of your proposed schedule compared against current disk state:
- *Keybinding:* Press =D= (or =C-c C-v=) in the review buffer.
- *Comprehensive Change Detection:* Displays categorized diff sections:
  - =- Task Name <original> + Task Name <proposed> (MOVED)=: Tasks moved to new time slots or days.
  - =+ Task Name <proposed> (NEWLY SCHEDULED)=: Unscheduled tasks assigned a slot.
  - =+ Task Name (SPLIT/PLACEHOLDER CHUNK)=: Temporary split or Pomodoro chunks.
  - =- Task Name (SKIPPED / UNCHECKED)=: Tasks toggled off in the review buffer.
  - =  Task Name (UNCHANGED)=: Preserved tasks.
- *Direct Navigation:* Press =RET= or =TAB= on any diff entry to jump directly to that task in the review buffer. Press =q= to dismiss.
...
| *Move Down / Later*                   | =J=                           | =J=                  | Move later in scheduling order (crosses days)           |
| *Chop / Slice Task*                   | =c=, =C-c c=                  |                      | Keep chosen duration today, defer rest to subsequent days |
| *What-If Diff Buffer*                 | =D=, =C-c C-v=                |                      | Inspect diff of proposed changes against disk state      |
...
Reflect on your planned vs. actual execution over the past week:
- *Access:* Press =W= in the dispatch menu (=M-x org-auto-scheduler-dispatch= or =C-c a W=).
- *Comprehensive Metrics:*
  - *Execution & Completion:* Total tasks scheduled vs. completed, completion percentage, and rolled tasks.
  - *Estimation Accuracy:* Planned effort vs. actual clocked hours, accuracy ratio, and estimation drift trends.
  - *Focus vs. Unplanned ("Squirrel" Factor):* Planned focus time vs. unplanned interruptive tasks with percentage split.
  - *Project & Category Breakdown:* Formatted ASCII breakdown table detailing completed count, clocked hours, and completion percentage per project.
