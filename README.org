...
| =p=                   | =review-pin-task=                            | Pin task to prompted start time (default: preview time; C-u p clears). 📌      |
| =F= / =i=             | =review-toggle-freeset=                      | Toggle FREESET (bypass workday hours; midnight split; no prompt). ⚡          |
| =s=                   | =review-toggle-splittable=                   | Toggle SPLITTABLE status of task (splits across gaps/days). [s]               |
...
All review mode keybindings (including =K=, =J=, =U=, =D=, =p=, =F=, =i=, =s=, =b=, =S=, =M=, =C=, =T=, and prefix keys =f= and =*=) are natively bound in Evil's =normal= and =motion= states, ensuring seamless modal editing without key collisions.
...
- *Toggle Shortcut:* Press =F= or =i= in the Review buffer to toggle =FREESET= without any prompt. Displays the =⚡ = badge in the review table.
...
- *Review Shortcut:* Press =p= in the Review buffer to pin a task. By default, the prompt is pre-filled with the task's current preview start time in the buffer.
- *Unpinning:* To unpin, press =C-u p= or submit an empty string at the prompt. Displays the =📌 = badge in the review table.
