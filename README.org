...
- [[#background-scheduling][Background Scheduling]]
  ...
  - [[#interactive-title-modifiers][Interactive Title Modifiers (-r-, -s-, -f-, -rsf-, -r-all-)]]
...
- *Interactive Mobile & CalDAV Title Modifiers:* Use quick title prefixes like =(-r-)=, =(-s-)=, =(-f-)=, =(-rsf-)=, or =(-r-all-)= to reschedule, enable splitting, or mark freeset from any mobile calendar without editing Org property drawers. Markers are automatically processed, applied, and stripped clean from the headline.
...
*** Interactive Title Modifiers (-r-, -s-, -f-, -rsf-, -r-all-)
...
| =-s-= or =(-s-)= | *Splittable* | Marks the task as =SPLITTABLE= (sets =:SPLITTABLE: t= and adds =:SPLITTABLE:= tag). |
| =-f-= or =(-f-)= | *Freeset* | Marks the task as =FREESET= (allows scheduling outside normal workday hours or across midnight). |
| Combinations | *Multi-Modifier* | Any permutation in any order (e.g. =(-rs-)=, =(-sfr-)=, =(-rf-)=, =-fr-=, =(-fsr-)=). Order does not matter! |
...
| =org-auto-scheduler-title-marker-regex=       | ="\\(?:(\\s-*\\)?-\\(r-all\\|[rRsSfF]+\\)-\\(?:\\s-*)\\)?"= | Regular expression for matching title-based modifiers like =(-r-)=, =(-sfr-)=, =(-r-all-)=. |
