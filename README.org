...
| =●=         | Color-coded dot representing the project. Fixed events show =📅= (blocking) or =🔓= (non-blocking).   |
...
| =Status=    | Status indicator: =✓= (success), =⚠= (warning), =✗= (failed), =🔒= (blocking), =🔓= (non-blocking).   |
...
- *In the Review Buffer:* Move point to the fixed event row and press =b= (or =SPC= / =m=). The icon changes from =📅= / =🔒= to =🔓= =[NB]=.
...
| =U= / =p= / =K=       | =review-move-up=                             | Move current task up in list (crosses day separators into previous day).       |
